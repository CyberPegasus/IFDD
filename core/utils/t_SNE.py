...
def _pca_eigen(X, no_dims=50):
    ...
    if int(torch.__version__.split('.')[0])==1 and int(torch.__version__.split('.')[1])<10:
        ...
        l = torch.stack([l.real,l.imag],dim=-1)
        M = M.real
...
# noinspection PyPep8Naming
def _tsne(X: Union[torch.Tensor, np.ndarray], no_dims: int = 2, initial_dims: int = 50, perplexity: float = 30.0, max_iter: int = 1000, verbose: bool = False,pca:str='eigen'):
    """
    ...
    if initial_dims < X.shape[1]:
        if pca == 'eigen':
            X = _pca_eigen(X, initial_dims)
        elif pca == 'svd':
            X = _pca_svd(X, initial_dims)
        else:
            raise NotImplementedError(f'pca method {pca} is not implemented.')
    elif verbose:
...
class TorchTSNE:
    def __init__(
            ...
            initial_dims: int = 50,
            verbose: bool = False,
            pca_method:str = 'eigen'  # 'eigen' or 'svd'
            # svd is faster, eigen is more accurate
    ):
