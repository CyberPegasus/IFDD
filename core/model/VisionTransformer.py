...
class GeneralAttention(nn.Module):
    ...
    def forward(self, x, context=None):
        ...
        kv = F.linear(input=x if context is None else context, weight=self.kv.weight)
        _, T2, _ = kv.shape
        kv = kv.reshape(B, T2, 2, self.num_heads, -1)
...
class VisionTransformer_Lift(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    def __init__(self, 
                 img_size=224, 
        ...
        if attn_type == 'joint':
            ...
            # The token type used for final classification
            self.lg_classify_token_type = lg_classify_token_type
            assert lg_classify_token_type in ['org', 'region', 'region1d', 'reg_org', 'all'], \
                f"Error: wrong 'lg_classify_token_type' in local_global attention ('{lg_classify_token_type}'), expected org / region / region1d / reg_org / all !"
        ...
        # ----- Lifting Scheme ----- #
        if self.use_WaveSF:
            from core.wave_methods.WaveLift_ViT import WaveSF_ViT_region,WaveSF_ViT_region_1D, WaveSF_ViT_org
            nt, t = self.lg_num_region_size[0], self.lg_region_size[0]
            nh, h = self.lg_num_region_size[1], self.lg_region_size[1]
            nw, w = self.lg_num_region_size[2], self.lg_region_size[2]
            if 'region' in self.lg_classify_token_type:
                input_size = [nt,nh,nw]
                if self.lg_classify_token_type == 'region':
                    self.Token_WaveLift = WaveSF_ViT_region(
                ...
                elif self.lg_classify_token_type == 'region1d':
                    self.Token_WaveLift = WaveSF_ViT_region_1D(
                        input_size=input_size,
                        embed_dim=embed_dim,
                        num_head=4,
                        relaxed_constrain=False,
                        change_x=False,
                        SSL=True,
                        WaveSF_preUpdate=WaveSF_preUpdate,
                        AdaptSplit=AdaptSplit,
                    ...
                    raise NotImplementedError()
            elif self.lg_classify_token_type == 'org' or self.lg_classify_token_type == 'reg_org':
                input_size = [nt,nh,nw,t,h,w]
                self.Token_WaveLift = WaveSF_ViT_org(
                    input_size=input_size,
                    embed_dim=embed_dim,
                    num_head=4,
                    relaxed_constrain=True,
                ...
                if self.lg_classify_token_type == 'reg_org':
                    self.head_org = nn.Linear(embed_dim, num_classes)
                    self.fc_norm_org = nn.LayerNorm(embed_dim)
            else:
                assert False, f'Not support {self.lg_classify_token_type} by Lift.'
        ...
        trunc_normal_(self.head.weight, std=.02)
        if self.lg_classify_token_type == 'reg_org':
            trunc_normal_(self.head_org.weight, std=.02)
        self.apply(self._init_weights)

        self.head.weight.data.mul_(init_scale)
        self.head.bias.data.mul_(init_scale)
        if self.lg_classify_token_type == 'reg_org':
            self.head_org.weight.data.mul_(init_scale)
            self.head_org.bias.data.mul_(init_scale)
    ...
    def forward_features(self, x):
        ...
        if self.attn_type == 'local_global':
            ...
            x = rearrange(x, '(b n) s c -> b n s c', b=b) # s = 1 + thw
            # token for final classification
            if 'region' in self.lg_classify_token_type: # only use region tokens for classification
                x = x[:,:,0] # (b, n, c)
            elif self.lg_classify_token_type == 'org': # only use original tokens for classification
                x = rearrange(x[:,:,1:], 'b n s c -> b (n s) c') # s = thw
            elif self.lg_classify_token_type == 'reg_org':
                x_reg = x[:,:,0].clone()
                x_org = rearrange(x[:,:,1:].clone(), 'b n s c -> b (n s) c') # s = thw
            else: # use all tokens for classification
                x = rearrange(x, 'b n s c -> b (n s) c') # s = 1 + thw
        ...
        if self.use_WaveSF:
            if 'region' in self.lg_classify_token_type:
                # b,n,c
                pool_a, x = self.Token_WaveLift(x,add_return=False)
                if self.lg_classify_token_type == 'region':
                    x = rearrange(x, 'b c t hw -> b (t hw) c')
            elif self.lg_classify_token_type == 'org':
                # b,n*s,c
                pool_a, x = self.Token_WaveLift(x,add_return=False)
                x = rearrange(x, 'b c t hw -> b (t hw) c')
            elif self.lg_classify_token_type == 'reg_org': # only use original tokens for classification
                pool_a, x = self.Token_WaveLift(x_org,add_return=False)
                x = rearrange(x, 'b c t hw -> b (t hw) c')
                x = torch.mean(x,dim=1,keepdim=True)
                x_reg = torch.mean(x_reg,dim=1,keepdim=True)
            else:
                assert False, f'Lifting not supports Token Type {self.lg_classify_token_type}'
        ...
        if self.fc_norm is not None:
            ...
            if self.keep_temporal_dim:
                ...
                if self.lg_classify_token_type == 'reg_org':
                    return self.fc_norm_org(x.mean(1)), self.fc_norm(x_reg.mean(1)), pool_a
                else:
                    return self.fc_norm(x.mean(1)), pool_a
        else:
            return x[:, 0]

    def forward(self, x, save_feature=False):
        if self.lg_classify_token_type == 'reg_org':
            x, x_reg, pool_a = self.forward_features(x)
        else:
            x, pool_a = self.forward_features(x)
        ...
        if self.lg_classify_token_type == 'reg_org':
            x = self.head(x_reg) + self.head_org(x)
        else:
            x = self.head(x)
        x = self.head_activation_func(x)
        if self.keep_temporal_dim:
            x = x.view(x.size(0), -1) # (B,T,C) -> (B,T*C)
        if save_feature:
            return x, pool_a, feature
        else:
            return x, pool_a
    
    @staticmethod
    def check_in(check_list:List[str],key:str):
        for i in range(len(check_list)):
            if key in check_list[i]:
                return i
        return -1
    ...
    def _frozen(self,freeze_list:List[str]):
        if 'backbone' in freeze_list:
            # freeze parameters
            self.lg_region_tokens.requires_grad = False
            # freeze models
            for name,module in self._modules.items():
                if (isinstance(module,nn.LayerNorm) or isinstance(module,nn.BatchNorm2d)):
                    # specific rule form "norm"
                    if name == 'norm':
                        if 'wave' in freeze_list:
                    ...
                    if not 'norm' in freeze_list:
                        ...
                        for param in module.parameters():
                            param.requires_grad = False
                        module.eval()
                elif 'Token_WaveLift' in name and not 'wave' in freeze_list:
                    pass
                elif name=='head' and not 'head' in freeze_list:
                    pass
                elif name=='head_org' and not 'wave' in freeze_list:
                    pass
                elif name == 'blocks' and self.check_in(freeze_list,'block') > -1:
                    i = self.check_in(freeze_list,'block')
                    unfreeze_layer_num = int(freeze_list[i].split('_')[-1])
                    for k in range(0,self.depth-unfreeze_layer_num):
                        for param in module[k].parameters():
                            param.requires_grad = False
                else:
                    for param in module.parameters():
                        param.requires_grad = False

def vit_backbone(num_cls:int, img_size:int,pretrained:str=None, **kwargs):
    ...
    if pretrained is not None:
        data = torch.load(pretrained, map_location='cpu')
        missing, unexpected = model.load_state_dict(data['model'],strict=False)
        logger.info(f"Load pretrained weights.\n Missing keys:{missing}\nUnexpected Keys:{unexpected}")
        
    return model

def vit_backbone_lift(num_cls:int, 
                      img_size:int,
                      pretrained:str,
                      use_WaveSF:bool,
                      WaveSF_preUpdate:bool,
                      AdaptSplit:bool, 
                      lift_source:str,
                      **kwargs):
    model = VisionTransformer_Lift(
        img_size=img_size,
        all_frames=16,
        patch_size=16, 
        num_classes = num_cls,
        ...
        lg_attn_param_sharing_all=False,
        lg_attn_param_sharing_first_third=False,
        lg_classify_token_type=lift_source, # region or org
        lg_first_attn_type='self',
        lg_third_attn_type='cross',
        lg_no_second=False,
        lg_no_third=False,
